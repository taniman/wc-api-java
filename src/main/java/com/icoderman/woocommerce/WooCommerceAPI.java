...
public class WooCommerceAPI implements WooCommerce {
    ...
    public WooCommerceAPI(OAuthConfig config, ApiVersionType apiVersion) {
        this.config = config;
        this.client = new com.icoderman.woocommerce.DefaultHttpClient();
    ...
    public Map get(String endpointBase, int id) {
        ...
        String signature = OAuthSignature.getAsQueryString(config, url, com.icoderman.woocommerce.HttpMethod.GET);
    ...
    public List getAll(String endpointBase, Map<String, String> params) {
        String url = String.format(API_URL_FORMAT, config.getUrl(), apiVersion, endpointBase);
        String signature = OAuthSignature.getAsQueryString(config, url, com.icoderman.woocommerce.HttpMethod.GET, params);
        String securedUrl = String.format(URL_SECURED_FORMAT, url, signature);
        return client.getAll(securedUrl);
    }

	@Override
	public String getAllRaw(String endpointBase, Map<String, String> params) {
		String url = String.format(API_URL_FORMAT, config.getUrl(), apiVersion, endpointBase);
		String signature = OAuthSignature.getAsQueryString(config, url, com.icoderman.woocommerce.HttpMethod.GET, params);
		String securedUrl = String.format(URL_SECURED_FORMAT, url, signature);
		return client.getAllRaw(securedUrl);
	}

    @Override
    public Map update(String endpointBase, int id, Map<String, Object> object) {
        String url = String.format(API_URL_ONE_ENTITY_FORMAT, config.getUrl(), apiVersion, endpointBase, id);
        return client.put(url, OAuthSignature.getAsMap(config, url, com.icoderman.woocommerce.HttpMethod.PUT), object);
    }

    @Override
    public Map delete(String endpointBase, int id) {
        String url = String.format(API_URL_ONE_ENTITY_FORMAT, config.getUrl(), apiVersion, endpointBase, id);
        Map<String, String> params = OAuthSignature.getAsMap(config, url, com.icoderman.woocommerce.HttpMethod.DELETE);
